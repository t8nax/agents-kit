...
# за все области. Оглавление подаёт хук, иначе сессия выбирала бы по памяти и пропуск прошёл бы
# молча.
...
Файла ``$path`` нет — задача в работе не числится. Память заводится по этому адресу со строкой ``рабочая копия: $worktree``, когда задача взята, — по правилам памяти задачи.
"@
    }
...
Основная копия ``$($state.workspace)`` объявляет базой ``$($state.base)``, но такого каталога нет.
...
В каталоге ``$($state.base)`` нет читаемого ``agents-kit.json`` с номером формата базы — это не база кита, а указатель на произвольный каталог.

**Работа со знанием остановлена.** Чинит оператор: переставить указатель или связать копию заново.
"@
        }

        # Сверка и подача знают только текущий формат: базу другого формата они прочли бы
        # сплошными находками, и сессия чинила бы их руками.
        'Outdated' {
            Emit @"
# agents-kit — база в прежнем формате
...
"@
        ...
        'Newer' {
            Emit @"
# agents-kit — база новее кита

Основная копия ``$($state.workspace)``: $(Get-KitFormatProblem $state).

**Работа со знанием остановлена:** этот кит прочёл бы базу по прежним правилам. Сказать оператору обновить кит и открыть новую сессию.
"@
        }

        'Unlisted' {
            Emit @"
# agents-kit — база не числит эту копию
...
"@
        ...
        'Linked' {
            ...
            $layoutLine = ''
            foreach ($ref in @(@('base-layout.md', 'Раскладка базы'), @('flow-stages.md', 'Правила флоу: сценарии и этапы'), @('task-memory.md', 'Правила памяти задачи'), @('backlog-record.md', 'Правила записи бэклога'), @('glossary.md', 'Глоссарий'))) {
            ...
            $mainLine = ''
            if ($state.worktree -ine $state.workspace) { $mainLine = "`n- Основная копия: ``$($state.workspace)``" }
            Emit @"
