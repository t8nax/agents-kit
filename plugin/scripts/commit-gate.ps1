...
#
# Проверяются файлы коммита и local/, а не база целиком: чужой перерасход не останавливает
# свой коммит. Команда разбирается, а не исполняется; не разобралась — коммит идёт, остальное
# назовёт сверка на старте.
...
Пропустить коммит решает оператор.
"@
    }
}
catch {
