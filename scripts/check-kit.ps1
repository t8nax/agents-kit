...
try {
    ...
    # по своему адресу, поэтому сессии называется не «решает оператор», а строка,
    ...
    # Ответ стоит последней строкой блока, а не сразу под заголовком: строки контекста
    # и вариантов между ними не должны прятать его от ожидания.
    Check 'ожидание — вопрос без ответа держит, ответ в конце блока отпускает' {
        $question = @(
            '- Оператору: Какой стенд дать под сверку?',
            '  - контекст: сверка накладных гоняется на стенде, доступа к нему у сессии нет',
        ...
        Set-Content -LiteralPath $script:memWt -Encoding utf8 -Value (@(
            '# Разбор накладной', "рабочая копия: $wt", '') + $question + @('', '## Шаги', '- [ ] прогнать сверку на стенде'))
        $p = Start-AwaitAnswer $script:memWt $wt
        try {
            if ($null -ne (Wait-AwaitAnswer $p 4)) { return 'вышло без ответа' }
            Set-Content -LiteralPath $script:memWt -Encoding utf8 -Value (@(
                '# Разбор накладной', "рабочая копия: $wt", '') + $question + @('  - ответ: общий', '', '## Шаги', '- [ ] прогнать сверку на стенде'))
