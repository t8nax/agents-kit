...
try {
    ...
    # Ответ стоит последней строкой блока, а не сразу под заголовком: контекст и варианты
    # между ними не должны прятать его от ожидания, а пустая строка ответа — отпускать.
    # За вопросом идёт часть «Агенту» со строкой про тот же вопрос: её «ответ:» — не ответ
    # оператора, и отпусти она ожидание, сессия проснулась бы без ответа.
