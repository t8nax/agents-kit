...
# вместе с нормализацией путей и порядком проверок. Дот-сорсится остальными
# скриптами, они только переводят состояние: разъедься они — новое состояние учли бы в одном
# и забыли в другом.
#
# Адрес базы — локальный git config копии, а не реестр маршрутов: реестр был машинно-зависимым
# файлом со строкой на копию, а git config не попадает в коммит и наследуется каждым worktree.
...
# то корневой ключ репозитория. Связь по каталогу — для монорепы: целиком под кит она не
# идёт, база одного модуля ехала бы в сессии чужих, а граница держалась бы дисциплиной. Копия, связанная целиком, работает как
