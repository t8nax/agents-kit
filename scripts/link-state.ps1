...
#
# Здесь лежит и нормализация путей, и порядок проверок. Разъехалась бы первая —
# сверка начала бы врать в обе стороны, то пропуская чужую копию, то блокируя свою;
# разъехался бы второй — новое состояние учли бы в одном вызывающем и забыли в другом.
