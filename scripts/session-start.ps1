...
База разошлась с раскладкой. **FAIL** — чинить до записи знания; перерасход, `local/` и память в коммите остановят и сам коммит в базу. **WARN** — перечитать и решить. Помеченное «решает человек» не трогать, а показать человеку.
