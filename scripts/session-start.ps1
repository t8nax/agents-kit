...
База разошлась с раскладкой. **FAIL** — чинить до записи знания; перерасход, `local/`, память, файл решений и сломанный флоу в коммите остановят и сам коммит в базу. **WARN** — перечитать и решить; похожее на секрет — строкой оператору. Помеченное «решает человек» не трогать, а показать человеку.
