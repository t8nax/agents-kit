...
Файла ``$path`` нет — задача в работе не числится. Память заводится по этому адресу, когда задача взята; как она ведётся и закрывается — правила памяти задачи.
"@
    }
...
**Содержимое не подано: это чужая работа.** Так выглядит файл памяти, положенный в базу руками, или две рабочие копии, чьи пути дали один адрес. Решает оператор.
"@
    }
...
База разошлась с правилами кита. **FAIL** — чинить до записи знания; перерасход, `local/`, память, файл решений и сломанный флоу в коммите остановят и сам коммит в базу. **WARN** — перечитать и решить; похожее на секрет — вопросом оператору. Помеченное «решает оператор» не трогать, а назвать вопросом оператору.
...
**Работа со знанием остановлена.** Другая база не подставляется, и знание не пишется никуда — в рабочий репозиторий в том числе. Чинит оператор: вернуть каталог базы на место или переставить указатель командой ``git config --local agents-kit.base <путь>``.
"@
        }
...
**Работа со знанием остановлена.** Так выглядит указатель, поставленный на произвольный каталог. Чинит оператор: переставить указатель или связать копию с базой заново.
"@
        }
...
**Работа со знанием остановлена.** Решает оператор: копия законная — связать её командой ``link.ps1 -Base "$($state.base)"``; копия случайная — снять указатель командой ``git config --local --unset agents-kit.base``.
"@
        }
        ...
        'Linked' {
            ...
            # Раскладка и правила памяти подаются путём, а не текстом: нужны они только той
            # сессии, что пишет в базу или в память. Путь считается в момент запуска и в
            # файлы кита не попадает.
            $layoutLine = ''
            foreach ($ref in @(@('base-layout.md', 'Раскладка базы'), @('task-memory.md', 'Правила памяти задачи'))) {
