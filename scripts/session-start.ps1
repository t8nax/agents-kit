...
Файла ``$path`` нет — задача в работе не числится. Память заводится по этому адресу, когда задача взята, и удаляется коммитом, которым задача закрыта.
"@
    }
...
База разошлась с раскладкой. **FAIL** — чинить до записи знания; перерасход, `local/`, память и сломанный флоу в коммите остановят и сам коммит в базу. **WARN** — перечитать и решить; похожее на секрет — строкой человеку. Помеченное «решает человек» не трогать, а показать человеку.
