...
База разошлась с раскладкой. **FAIL** — чинить до записи знания; перерасход, `local/`, память, файл решений и сломанный флоу в коммите остановят и сам коммит в базу. **WARN** — перечитать и решить; похожее на секрет — вопросом оператору. Помеченное «решает оператор» не трогать, а назвать вопросом оператору.
