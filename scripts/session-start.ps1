...
# Решения подаются оглавлением; почему — CLAUDE.md, собирает его base-check.ps1.
...
``$dir``, по файлу на область, содержимым не поданы. До первого шага задачи прочитать те, которых она касается, — раздел «Решения» раскладки.
...
"@
    ...
    # Неопознанная память не подаётся: подать чужую опаснее, чем не подать; почему — CLAUDE.md.
...
В файле ``$path`` нет строки ``рабочая копия: <путь>``, сверить его с копией нечем, и **содержимое не подано.** Файл лежит по адресу этой копии: дописать в него ``рабочая копия: $worktree`` и продолжить работу.
"@
    }
...
**FAIL** — чинить до записи знания; в файле коммита он остановит и сам коммит. **WARN** — перечитать и решить. Похожее на секрет и помеченное «решает оператор» не трогать, а назвать вопросом оператору.
