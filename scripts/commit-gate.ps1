...
# находок в решение: FAIL — коммит не идёт, похожее на секрет — решает человек,
# остальное — молчание.
# Проверяются только файлы коммита: чужой перерасход в общей базе не останавливает
# свой коммит, а чинить чужое сессия права не имеет.
#
# Гейт живёт в ките, а не в git базы: хук git требовал бы пути к киту в конфиге базы,
# а при установке маркетплейсом этот путь несёт версию и молча протухает.
#
# Команда разбирается по словам, а не исполняется, и разбор не всесилен. Не разобрал —
# молчит: пропущенное назовёт сверка на старте следующей сессии, а сломанный хук не
# должен ломать сессию. Почему ошибка гасится — CLAUDE.md.
...
try {
    ...
    # Прочие WARN коммит не останавливают и человека не дёргают: их назовёт сверка на
    # старте. Человеку несётся только подозрение на секрет — его последствия необратимы.
...
Починить названное и повторить коммит. Помеченное «решает человек» не трогать, а показать человеку.
"@
    }
...
Секрет, попавший в историю базы, удалением файла не убрать. Пропустить коммит решает человек.
"@
    }
}
catch {
