...
#
# Гейт — хук Claude Code, а не хук git базы: хуку git нужен путь к киту в конфиге базы, а при
# установке маркетплейсом путь несёт версию и молча протухает. Проверяются файлы коммита и
# local/, а не база целиком: чужой перерасход не останавливает свой коммит. Команда
# разбирается, а не исполняется; не разобралась — коммит идёт, остальное назовёт сверка на старте.
...
"@
    ...
    # Молчание безопаснее полуправды: сломанный гейт не ломает сессию.
}
exit 0
